...
contract Token is BEP20('BRAVE', 'BRV') {
    using SafeMath for uint256;
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    //Anti-Bot
    uint256 public minHarvestTime;
    uint256 public maxHarvestTime;
    mapping(address => mapping(uint256 => uint256)) harvestTimeout;
    ...
    constructor(
        ...
        minHarvestTime = 1 hours;
        maxHarvestTime = 4 hours;
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                uint256 antiBotFees;
                if (block.timestamp < harvestTimeout[msg.sender][_pid].add(minHarvestTime)){
                    antiBotFees = pending.mul(10).div(1);
                } else if (block.timestamp < harvestTimeout[msg.sender][_pid].add(minHarvestTime).add(maxHarvestTime.div(3))){
                    antiBotFees = pending.mul(20).div(1);
                }else if (block.timestamp < harvestTimeout[msg.sender][_pid].add(minHarvestTime).add(maxHarvestTime.div(3).mul(2))){
                    antiBotFees = pending.mul(40).div(1);
                }else if (block.timestamp < harvestTimeout[msg.sender][_pid].add(minHarvestTime).add(maxHarvestTime)){
                    antiBotFees = pending.mul(80).div(1);
                }

                safeEggTransfer(msg.sender, pending.sub(antiBotFees));
                safeEggTransfer(address(0x000000000000000000000000000000000000dEaD), antiBotFees);
            }
            harvestTimeout[msg.sender][_pid]=block.timestamp;
        }
        if (_amount > 0) {
            ...
            if (pool.depositFeeBP > 0) {
                ...
                if (_pid == 3){
                    pool.lpToken.safeTransfer(address(0x000000000000000000000000000000000000dEaD), depositFee);
                } else {
